...
export async function GET(
  ...
  try {
    ...
    // Get filename from query params, fallback to attachmentId
    const filename = req.nextUrl.searchParams.get("filename") || attachmentId;

    // Use userId in object key for proper access control
    const objectKey = `${userId}/${attachmentId}`;
    ...
    console.error("Error generating download URL for attachment:", error);
    return NextResponse.json(
